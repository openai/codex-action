...
- **`drop-sudo` (default)** — On Linux and macOS runners, the action revokes the default user’s `sudo` membership before invoking Codex. On Linux, Codex runs without supplementary groups and cannot access the Docker daemon, while later workflow steps retain Docker access. Removing `sudo` lasts for the rest of the job, so subsequent steps cannot rely on it. This is usually the safest choice on GitHub-hosted runners.
...
- **Linux/macOS**: All options for `safety-strategy` are supported. Linux `drop-sudo` requires passwordless sudo and `/usr/bin/setpriv` before Codex starts, and only Codex commands launched by this action run without Docker access. Again, if you pick `drop-sudo`, remember that later steps in your `job` that rely on `sudo` will fail. If you do need to run code that requires `sudo` after `openai/codex-action` has run, one option is to pipe the output of `openai/codex-action` to a fresh `job` on a new host and to continue your workflow from there.
...
- Once `openai/codex-action` is run once with `openai-api-key`, you can also call `codex` from subsequent scripts in your job. However, direct Linux commands retain Docker access and do not receive the action's supplementary-group protection; provide `prompt` or `prompt-file` to run Codex safely through the action.
