...
| `azure-api-key` | Secret used to start the Responses API proxy when present; if non-empty the action assumes Azure mode. Store it in `secrets`.                                 | `""`        |
...
- To use Azure OpenAI, supply both `azure-api-key` and `azure-url`; the action will automatically route requests to Azure when the key is present.
