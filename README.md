...
| `sandbox`                | Legacy sandbox mode. One of `workspace-write`, `read-only` or `danger-full-access`. Mutually exclusive with `permission-profile`.              | `""`        |
| `permission-profile`     | Built-in or configured Codex permission profile selected through `default_permissions`. Mutually exclusive with `sandbox`.                     | `""`        |
...
Codex permission profiles independently describe filesystem and network access. Set
`permission-profile` to a built-in profile such as `:read-only` or `:workspace`, or to a named
profile defined in the `config.toml` under `codex-home`. See the
...
For example, after defining `public-review` in the selected Codex home:
...
- If you need more control over the CLI invocation, pass flags through `codex-args` or create a `config.toml` in `codex-home`. Prefer `permission-profile` over legacy sandbox flags for new integrations.
