...
| `azure-api-key` | Secret used to start the Responses API proxy when present; if non-empty the action assumes Azure mode. The proxy still authenticates with `Authorization: Bearer <key>`. Store it in `secrets`. | `""`        |
| `azure-url` | Full Azure Responses API URL, e.g. `https://example.openai.azure.com/openai/deployments/my-deployment/responses?api-version=2025-04-01-preview`. Required when `azure-api-key` is provided. | `""`        |
...
- To use Azure OpenAI, supply both `azure-api-key` and `azure-url`; the action will automatically route requests to Azure when the key is present while continuing to send `Authorization: Bearer <key>` upstream.
