...
complete the task. Permission profiles constrain commands that Codex runs; they do not replace the
action's `safety-strategy`, which controls the privileges of the Codex process itself. Continue to
use `drop-sudo` or a deliberately configured `unprivileged-user` when a profile grants filesystem
